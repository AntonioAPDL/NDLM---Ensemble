...
-\frac12\sum_{t=T+1}^{T+K}\!\left[
\log|\bm W_t^\beta|+(\bm e_t^\beta)^\T(\bm W_t^\beta)^{-1}\bm e_t^\beta
\right] \nonumber\\
&-\frac12\sum_{t=T+1}^{T+K}\sum_{n=1}^{n_t}\!\left[
\log \sigma^2_{\mathrm{src}(t,n)}+\frac{(r_{t,n}^{C})^2}{\sigma^2_{\mathrm{src}(t,n)}}
\right],
\label{eq:log_joint_C}
\end{align}
where $\mathrm{src}(t,n)$ maps each scalar observation to its source-variance index.
