...
source-specific horizons $K_j$:
\begin{align}
\mathcal A_k &:= \{j: k\le K_j\}, \qquad J_k := |\mathcal A_k|, \qquad k=1,\dots,K_{\max},
\end{align}
with $K_{\max}:=\max_j K_j$ and state dimension $p_k:=q(1+J_k)$.
...
\begin{align}
 ...
 \bm\beta_{T+k}\given\bm\beta_{T+k-1}
 &\sim \N(\bm M_k^\star \bm\beta_{T+k-1},\bm W_k^\star). \label{eq:C_state}
\end{align}
Here $(\bm M_k^\star,\bm W_k^\star)$ are the transdimensional transition blocks.
Equivalent fixed-dimension implementations may use an embedding state
$\tilde{\bm\beta}_{T+k}\in\R^{q(1+J_f)}$ with deterministic selection matrix
$\bm S_k\in\{0,1\}^{p_k\times q(1+J_f)}$ such that
$\bm\beta_{T+k}=\bm S_k\tilde{\bm\beta}_{T+k}$.
