...
$\mathcal A_k=\{j:k\le K_j\}$ induces state dimension $p_k=q(1+|\mathcal A_k|)$.
Two equivalent implementations are valid:
\begin{enumerate}
\item True transdimensional recursion in $\R^{p_k}$ with lead-dependent
      $(\bm G_k,\bm Q_k,\bm h_{k,n})$.
\item Fixed-dimension embedding $\tilde{\bm\beta}_{T+k}\in\R^{q(1+J_f)}$ with
      deterministic selection/projection matrices $\bm S_k$ such that
      $\bm\beta_{T+k}=\bm S_k\tilde{\bm\beta}_{T+k}$ and forecast likelihoods
      only use active coordinates.
\end{enumerate}
