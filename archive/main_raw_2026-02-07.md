...
#### Forecast-period covariate path

To allow the transfer-function block to be retained during forecasting, define a supplied forecast-period covariate path (x_t^\star \in \mathbb{R}^{m}) for (t=T+1,\dots,T+K), where
[
\boxed{
x_t^\star =
\begin{cases}
x_t, & \text{if realized future covariates are available},\\
\widehat{x}_{t\mid T}, & \text{if deterministic covariate forecasts are supplied}.
\end{cases}
}
]
Forecast-period inference with the transfer-function block is conditional on the supplied path ({x_t^\star}_{t=T+1}^{T+K}).

#### Forecast stacked state (reduced specification)
...
This is the current no-transfer forecast specification.

#### Forecast stacked state (augmented specification; transfer retained)

If the transfer-function block is retained in the forecast period, define
[
\boxed{
\widetilde{\beta}_t :=
\begin{pmatrix}
...
\psi_t
\end{pmatrix}
\in \mathbb{R}^{q(1+J_f)+1+m},
\qquad t=T,\dots,T+K.
}
]
The reduced forecast specification is recovered by omitting the transfer-function block ((\zeta_t,\psi_t)) when no forecast-period covariate path is supplied.
...
At forecast time (t=T+k), for product (j\in\{1,\dots,J_f\}), define the reduced forecast selector/design vector (e_{t,j}\in\mathbb{R}^{q(1+J_f)}) such that
[
\boxed{
\mu_t^{(j),\mathrm{red}} := e_{t,j}^\top \beta_t,
...
where the second (F_t) sits in the (j)-th discrepancy block. (If your forecast design differs from (F_t), replace (F_t) by the forecast design (F_t^{(f)}) consistently.)
...
[
\boxed{
\mu_t^{(j),\mathrm{aug}} := \widetilde e_{t,j}^\top \widetilde\beta_t
= e_{t,j}^\top \beta_t + \zeta_t,
\qquad t=T+1,\dots,T+K.
}
]
...
x_t^{(f)} :=
\begin{cases}
\beta_t, & \text{reduced forecast specification},\\
\widetilde\beta_t, & \text{augmented forecast specification},
\end{cases}
\qquad
h_{t,j}^{(f)} :=
\begin{cases}
e_{t,j}, & \text{reduced forecast specification},\\
\widetilde e_{t,j}, & \text{augmented forecast specification},
\end{cases}
}
]
so that the product-specific forecast latent mean can always be written as
[
\boxed{
\mu_t^{(j)} = h_{t,j}^{(f)\top} x_t^{(f)}.
}
]
...
Two forecast specifications are useful.

**Reduced forecast specification.** For each (k\in\{1,\dots,K(j)\}), member (i\in\{1,\dots,I_j\}), and (j=1,\dots,J_f),
[
\boxed{
...
**Augmented forecast specification (transfer retained).** Conditional on the supplied covariate path, for each (k\in\{1,\dots,K(j)\}), member (i\in\{1,\dots,I_j\}), and (j=1,\dots,J_f),
[
\boxed{
y_T^{j,i}(k)\mid \widetilde\beta_{T+k},\sigma_j^2,x_{T+k}^\star
\sim N(\widetilde e_{T+k,j}^\top \widetilde\beta_{T+k},\ \sigma_j^2).
}
]

Conditional independence across ensemble members holds given the relevant forecast state and ((\sigma_j^2)); in the augmented specification this is conditional on the supplied covariate path.
...
Typically (G_t^\beta) is block-diagonal with repeated (G_t) on each (q)-block; likewise (W_t^\beta) is block-diagonal with corresponding covariance blocks (baseline and each discrepancy).

#### Forecast stacked state (augmented specification; transfer retained)

If the transfer-function block is retained during forecasting, then for forecast times (t=T+1,\dots,T+K), conditional on the supplied covariate path, write
[
\boxed{
\widetilde\beta_t \mid \widetilde\beta_{t-1},x_t^\star
\sim N(\widetilde G_t^\beta \widetilde\beta_{t-1},\ \widetilde W_t^\beta).
}
]
One common block representation is
[
\widetilde G_t^\beta=
\begin{pmatrix}
G_t^\beta & 0 & 0\
0 & \lambda & x_t^{\star\top}\
0 & 0 & I_m
\end{pmatrix},
\qquad
\widetilde W_t^\beta=
...
This preserves the current reduced forecast model as a special case, while allowing the transfer-function block to be propagated whenever a forecast-period covariate path is supplied.
...
For the forecast stage, use
[
\boxed{
\beta_T \sim N(m_T^\beta,C_T^\beta)
\quad\text{(reduced forecast specification)},
}
]
or
[
\boxed{
\widetilde\beta_T \sim N(m_T^{\widetilde\beta},C_T^{\widetilde\beta})
\quad\text{(augmented forecast specification)}.
}
]
If (\beta_T) or (\widetilde\beta_T) is constructed from historical filtering/smoothing at (t=T), then the corresponding terminal moments are induced rather than separately specified; state explicitly which convention you adopt.
...
* Forecast stacked trajectory (reduced specification): (\beta_{T:T+K}:={\beta_t}_{t=T}^{T+K}).
* Forecast stacked trajectory (augmented specification): (\widetilde\beta_{T:T+K}:={\widetilde\beta_t}_{t=T}^{T+K}).

Define data:
[
\mathcal{D}_{\text{hist}} := {y_t^0}_{t=1}^T \cup {z_{jt}}_{j=1,\dots,J;\ t=1,\dots,T},
\qquad
\mathcal{D}_{\text{fcast}} := {y_T^{j,i}(k)}.
]
When the augmented forecast specification is used, forecast-period inference is conditional on the supplied covariate path ({x_t^\star}_{t=T+1}^{T+K}).
...
### 2.5 Model C: forecast-period ensemble likelihood and forecast-state specification

This section defines the joint density for the forecast period. Two forecast specifications are useful:

* **Reduced Model C:** the forecast state is (\beta_t) and the transfer-function block is omitted after time (T).
* **Augmented Model C:** the forecast state is (\widetilde\beta_t) and the transfer-function block is retained, conditional on the supplied forecast-period covariate path ({x_t^\star}_{t=T+1}^{T+K}).

The current no-transfer forecast model is the reduced specification; the augmented specification is its direct extension.

As before, two conventions are common:

* **(C1) Conditional forecast model given a terminal distribution at (t=T)**: treat (\beta_T) or (\widetilde\beta_T) as given by historical filtering/smoothing (random with known terminal Gaussian moments).
* **(C2) Fully joint across historical and forecast**: define the terminal forecast state deterministically from historical states (or include coupling constraints) and sample jointly.

Below is the factorization for **(C1)**.

**Reduced forecast likelihood.** Let (t=T+k). For each scalar ensemble observation at time (t),
[
...
**Full joint (reduced Model C; conditional on terminal prior).**
Let (\Theta_C^{\mathrm{red}} := ({\sigma_j^2}_{j=1}^{J_f},\mathcal{W}_\beta)). Then
[
\boxed{
\begin{aligned}
p(\mathcal{D}_{\text{fcast}},\beta_{T:T+K},\Theta_C^{\mathrm{red}})
&=
...
\phi_{d_\beta}\!\left(\beta_t;\ G_t^\beta\beta_{t-1},\ W_t^\beta\right) \\
&\quad\times
\prod_{k=1}^{K}\prod_{j=1}^{J_f}\prod_{i=1}^{I_j}
\phi\!\left(y_T^{j,i}(k);\ e_{T+k,j}^\top\beta_{T+k},\ \sigma_j^2\right).
\end{aligned}
}
]

**Augmented forecast likelihood.** Let (t=T+k). Conditional on the supplied covariate path,
[
p(\mathcal{Y}_{t}\mid \widetilde\beta_t,{\sigma_j^2},x_t^\star)
=
\prod_{j=1}^{J_f}\prod_{i=1}^{I_j}
\phi\!\left(y_T^{j,i}(k);\ \widetilde e_{t,j}^\top\widetilde\beta_t,\ \sigma_j^2\right).
]

**Augmented forecast evolution.**
[
p(\widetilde\beta_{T:T+K}\mid \mathcal{W}_{\widetilde\beta},\lambda,{x^\star}_{T+1:T+K})
=
p(\widetilde\beta_T)\prod_{t=T+1}^{T+K}
\phi_{d_{\widetilde\beta}}\!\left(\widetilde\beta_t;\ \widetilde G_t^\beta\widetilde\beta_{t-1},\ \widetilde W_t^\beta\right),
\qquad d_{\widetilde\beta}=q(1+J_f)+1+m.
]

**Full joint (augmented Model C; conditional on terminal prior).**
Let (\Theta_C^{\mathrm{aug}} := ({\sigma_j^2}_{j=1}^{J_f},\lambda,\mathcal{W}_{\widetilde\beta})). Then
[
\boxed{
\begin{aligned}
&p(\mathcal{D}_{\text{fcast}},\widetilde\beta_{T:T+K},\Theta_C^{\mathrm{aug}} \mid {x^\star}_{T+1:T+K}) \\
&=
\Big[\prod_{j=1}^{J_f} p(\sigma_j^2)\Big]\,
p(\lambda)\,
p(\mathcal{W}_{\widetilde\beta})\,
p(\widetilde\beta_T)\,
\prod_{t=T+1}^{T+K}
\phi_{d_{\widetilde\beta}}\!\left(\widetilde\beta_t;\ \widetilde G_t^\beta\widetilde\beta_{t-1},\ \widetilde W_t^\beta\right) \\
&\quad\times
\prod_{k=1}^{K}\prod_{j=1}^{J_f}\prod_{i=1}^{I_j}
\phi\!\left(y_T^{j,i}(k);\ \widetilde e_{T+k,j}^\top\widetilde\beta_{T+k},\ \sigma_j^2\right).
\end{aligned}
}
]

If (x_t^\star=\widehat x_{t\mid T}) is a deterministic forecast path, then the resulting forecast distribution is conditional on that supplied path and does not propagate uncertainty in the covariate forecasts.

**Optional (C2) coupling note.** If (\beta_T) or (\widetilde\beta_T) is constructed from the historical states coming from Model B, then replace the terminal prior by the induced distribution (or a coupling constraint) and write a single joint density over the corresponding historical and forecast trajectories. In that case, Model C becomes a continuation of the same Markov chain from (t=T) onward.
...
* Section 1: definitions of (\alpha_t,\delta_t^j,\beta_t,\widetilde\beta_t), observation means (\tilde F_t^\top\alpha_t), (\tilde F_t^\top\alpha_t+F_t^\top\delta_t^j), and (\mu_t^{(j)} = h_{t,j}^{(f)\top}x_t^{(f)}); Gaussian likelihood statements; Gaussian transition models.
...
Its dimension is (d_C=q(1+J_f)) in the reduced case and (d_{\widetilde C}=q(1+J_f)+1+m) in the augmented case.
...
* **Forecast ensemble ((j=1,\dots,J_f))** (forecast period):
  ...
  [
  e^{\text{fcast}}_{j,i,k} := y_T^{j,i}(k) - h_{T+k,j}^{(f)\top} x_{T+k}^{(f)},
...
\mathcal{T}_\psi :=
\begin{cases}
\{1,\dots,T\}, & \text{historical-only or reduced forecast specification},\\
\{1,\dots,T+K\}, & \text{augmented forecast specification}.
\end{cases}
]
With (\psi_t \mid \psi_{t-1}\sim N(\psi_{t-1}, W^\psi)) (random walk) and IW prior:
[
u_t^\psi := \psi_t - \psi_{t-1},\quad t\in\mathcal{T}_\psi,
]
[
\boxed{
W^\psi \mid \cdot \sim \mathrm{IW}\!\left(\nu_\psi + |\mathcal{T}_\psi|,\ S_\psi + \sum_{t\in\mathcal{T}_\psi} u_t^\psi (u_t^\psi)^\top\right).
}
]
...
With (\zeta_t \mid (\zeta_{t-1},\psi_t)\sim N(\lambda\zeta_{t-1}+x_t^{(\lambda)\top}\psi_t,\ w^\zeta)) and IG prior (w^\zeta\sim \mathrm{IG}(a_\zeta,b_\zeta)), define
[
u_t^\zeta := \zeta_t - \lambda \zeta_{t-1} - x_t^{(\lambda)\top} \psi_t,\quad t\in\mathcal{T}_\zeta,
]
and update
[
\boxed{
w^\zeta \mid \cdot \sim \mathrm{IG}\!\left(a_\zeta + \frac{|\mathcal{T}_\zeta|}{2},\ b_\zeta + \frac{1}{2}\sum_{t\in\mathcal{T}_\zeta} (u_t^\zeta)^2\right).
}
]
...
#### 4.2.4 Forecast evolution covariance (forecast-period state)

For the reduced forecast specification, if you treat the forecast stacked state as
[
...
For the augmented forecast specification, if you instead use
[
\widetilde\beta_t\mid \widetilde\beta_{t-1},x_t^\star
\sim N(\widetilde G_t^\beta \widetilde\beta_{t-1}, \widetilde W_t^\beta),
\quad t=T+1,\dots,T+K,
...
define
[
u_t^{\widetilde\beta} := \widetilde\beta_t - \widetilde G_t^\beta \widetilde\beta_{t-1},\quad t=T+1,\dots,T+K.
]
If a single forecast covariance block (\widetilde W^\beta) is used with prior (\widetilde W^\beta\sim\mathrm{IW}(\nu_{\widetilde\beta},S_{\widetilde\beta})), then
[
\boxed{
\widetilde W^\beta \mid \cdot \sim \mathrm{IW}\!\left(\nu_{\widetilde\beta} + K,\ S_{\widetilde\beta} + \sum_{t=T+1}^{T+K} u_t^{\widetilde\beta}(u_t^{\widetilde\beta})^\top\right).
}
]
If instead (\widetilde W_t^\beta) is parameterized blockwise as (\operatorname{BlockDiag}(W_t^\beta,w_t^\zeta,W_t^\psi)), then update the corresponding blocks ((W^\beta,w^\zeta,W^\psi)) using their own innovation sums.

---

### 4.3 Transfer coefficient (\lambda): Normal (or truncated Normal) update

Assume (\lambda) is unknown and has prior (\lambda\sim N(m_\lambda,s_\lambda^2)). Condition on the sampled trajectories ({\zeta_t,\psi_t}_{t=0}^{T_\lambda}) and on (w^\zeta) (or ({w_t^\zeta})), where
[
\boxed{
T_\lambda :=
\begin{cases}
T, & \text{historical-only or reduced forecast specification},\\
T+K, & \text{augmented forecast specification}.
\end{cases}
}
]
Define
...
x_t^\star, & t>T,
\end{cases}
\qquad t=1,\dots,T_\lambda.
]

From the transfer evolution:
[
\zeta_t = \lambda \zeta_{t-1} + x_t^{(\lambda)\top} \psi_t + \eta_t,\qquad \eta_t\sim N(0,w^\zeta),
]
define the “regression response”
[
y_t^\lambda := \zeta_t - x_t^{(\lambda)\top} \psi_t.
]
Then
[
...
[
S_{xx} := \sum_{t=1}^{T_\lambda} \zeta_{t-1}^2,\qquad
S_{xy} := \sum_{t=1}^{T_\lambda} \zeta_{t-1} y_t^\lambda.
]
...
[
S_{xx} := \sum_{t=1}^{T_\lambda} \frac{\zeta_{t-1}^2}{w_t^\zeta},\qquad
S_{xy} := \sum_{t=1}^{T_\lambda} \frac{\zeta_{t-1} y_t^\lambda}{w_t^\zeta}.
]
...
* Section 1: definitions of observation means and evolution equations for (\theta_t,\zeta_t,\psi_t,\delta_t^j,\beta_t,\widetilde\beta_t).
* Section 3: FFBS provides sampled trajectories used to compute residuals and innovations.
...
* Model C: (\mathcal{D}={y_T^{j,i}(k)}) together with either an initial prior for (\beta_T) (reduced forecast specification) or an initial prior for (\widetilde\beta_T) (augmented forecast specification, conditional on the supplied covariate path).
...
* Data: ({y_t^0}_{t=1}^T), ({z_{jt}}_{j=1..J,t=1..T}), and optionally ({y_T^{j,i}(k)}).
* Known design/evolution objects: (\tilde F_t, F_t, \tilde G_t(\lambda), G_t, G_t^\beta, e_{t,j}), and, when the transfer-function block is retained during forecasting, (\widetilde G_t^\beta, \widetilde e_{t,j}, x_t^\star).
...
* **Model C (forecast period; conditional convention):**
  given a terminal prior for the relevant forecast state, draw
  [
  x_{T:T+K}^{(f),(s)} \sim p(x_{T:T+K}^{(f)}\mid {y_T^{j,i}(k)},\Theta^{(s-1)}),
  ]
  via FFBS, where
  [
  x_t^{(f)} =
  ...
  Assimilate all ensemble-member observations at each (t=T+k) sequentially using the corresponding forecast design ((h_{t,j}^{(f)}=e_{t,j}) or (\widetilde e_{t,j})).
...
3. **Coupling between Model B and Model C**
   If you later choose a *fully joint* sampler across historical and forecast, replace the conditional Model C step by an FFBS draw on a single time-extended state. The same idea applies to the reduced and augmented forecast specifications; the only difference is whether the forecast-period state is (\beta_t) or (\widetilde\beta_t), and whether the supplied covariate path ({x_t^\star}) is used after time (T).
...
* “states” means (\alpha_{0:T}) for Model A, the stacked (x_{0:T}=(\alpha_{0:T},\delta^{1:J}_{0:T})) for Model B, and either (\beta_{T:T+K}) or (\widetilde\beta_{T:T+K}) for Model C, depending on whether the reduced or augmented forecast specification is used.
...
If (w^\zeta\sim \mathrm{IG}(a_\zeta,b_\zeta)) and
[
u_t^\zeta := \zeta_t-\lambda\zeta_{t-1}-x_t^{(\lambda)\top}\psi_t,
\qquad t\in\mathcal{T}_\zeta,
]
then
[
\boxed{
...
\quad
a_\zeta^\star \leftarrow a_\zeta+\frac{|\mathcal{T}_\zeta|}{2},
\quad
b_\zeta^\star \leftarrow b_\zeta + \frac12 \sum_{t\in\mathcal{T}_\zeta} \mathbb{E}_q[(u_t^\zeta)^2],
}
]
...
with (w^\zeta) either fixed or having variational factor (q(w^\zeta)). Define
[
\mathcal{T}_\lambda :=
\begin{cases}
\{1,\dots,T\}, & \text{historical-only or reduced forecast specification},\\
...
Define (y_t^\lambda := \zeta_t - x_t^{(\lambda)\top}\psi_t) for (t\in\mathcal{T}_\lambda). Under mean-field, the optimal (q(\lambda)=N(m_\lambda,v_\lambda)) satisfies
[
\boxed{
v_\lambda^{-1}
\leftarrow
s_{\lambda 0}^{-2} + \mathbb{E}_q[(w^\zeta)^{-1}] \sum_{t\in\mathcal{T}_\lambda} \mathbb{E}_q[\zeta_{t-1}^2],
}
]
[
\boxed{
...
+
\mathbb{E}_q[(w^\zeta)^{-1}]
\sum_{t\in\mathcal{T}_\lambda} \mathbb{E}_q[\zeta_{t-1} y_t^\lambda]
\right).
}
]
All expectations are computable from the smoothed moments of ((\zeta_t,\psi_t)):
[
\mathbb{E}_q[\zeta_{t-1}^2]=\mathrm{Var}_q(\zeta_{t-1})+(\mathbb{E}_q[\zeta_{t-1}])^2,
]
and similarly
[
\mathbb{E}_q[\zeta_{t-1} y_t^\lambda]
=
\mathbb{E}_q[\zeta_{t-1}\zeta_t]
-
\mathbb{E}_q[\zeta_{t-1} x_t^{(\lambda)\top}\psi_t],
]
where (\mathbb{E}_q[\zeta_{t-1}\zeta_t]) and cross-moments with (\psi_t) come from (C_{t,t-1}) and within-time covariances.
...
**Model C (forecast ensembles):**
At (t=T+k), use the forecast observation list
[
\mathcal{O}_{T+k}
=
\{(y_T^{j,i}(k),\ h_{T+k,j}^{(f)},\ \sigma_j^2)\}_{j=1..J_f,\ i=1..I_j},
]
where (h_{t,j}^{(f)}=e_{t,j}) in the reduced forecast specification and (h_{t,j}^{(f)}=\widetilde e_{t,j}) in the augmented forecast specification.
...
* Model C has block-diagonal or block-structured forecast evolution matrices: in the reduced specification this is typically ((G_t^\beta,W_t^\beta)), while in the augmented specification it is ((\widetilde G_t^\beta,\widetilde W_t^\beta)).
...
Use the forecast state ((x_{T+k}^{(f)})) and forecast design ((h_{T+k,j}^{(f)})) from Section 1:
[
\boxed{
y_T^{j,i}(k)\mid x_{T+k}^{(f)},\sigma_j^2 \sim N(h_{T+k,j}^{(f)\top}x_{T+k}^{(f)},\sigma_j^2).
}
]
This covers both the reduced forecast specification ((x_t^{(f)}=\beta_t,\ h_{t,j}^{(f)}=e_{t,j})) and the augmented forecast specification ((x_t^{(f)}=\widetilde\beta_t,\ h_{t,j}^{(f)}=\widetilde e_{t,j})). Posterior predictive for any ensemble summary (mean/spread/quantiles) is obtained by Monte Carlo over (s) (MCMC) or Normal approximation (VB) using the same (h).
...
* Posterior for latent mean (m_t^{(j)}:=h_{t,j}^{(f)\top}x_t^{(f)}) (MCMC draws or VB Normal).
* Predictive distribution for an individual ensemble member observation:
  [
  N\!\big(h_{t,j}^{(f)\top}x_t^{(f)},\ \sigma_j^2\big).
  ]
* Predictive distribution for a new replicate observation under the corrected model (using §§9.3–9.4).

If you also want a target forecast (y_{T+k}^0), use the reduced or augmented target design described in Section 10.3. Both cases remain linear-Gaussian and fit the same machinery.
...
\[
y_T^{j,i}(k)\mid x_t^{(f)},\sigma_j^2 \sim N(h_{t,j}^{(f)\top} x_t^{(f)},\ \sigma_j^2),
...
\[
\boxed{
\bar y_{t,j} \mid x_t^{(f)},\sigma_j^2 \sim N\!\left(h_{t,j}^{(f)\top} x_t^{(f)},\ \frac{\sigma_j^2}{I_{t,j}}\right),
\qquad j=1,\dots,J_f,
}
\]
...
With \(\eta_{t,j}:=h_{t,j}^{(f)\top} x_t^{(f)}\),
\[
\boxed{
...
\qquad
\bar y_{t,j} \mid x_t \approx N\!\left(h_{t,j}^{(f)\top} x_t,\ \tilde r_{t,j}^{\text{agg}}\right),
}
\]
where \(h_{t,j}^{(f)}\) is \(e_{t,j}\) in the reduced forecast specification and \(\widetilde e_{t,j}\) in the augmented forecast specification.

For the expected SSE used in the \(q(\sigma_j^2)\) update (Section 6.3), let \(q(x_t^{(f)})=N(m_t,C_t)\) and define
\(\mu_{t,j}:=h_{t,j}^{(f)\top} m_t\), \(v_{t,j}:=h_{t,j}^{(f)\top} C_t h_{t,j}^{(f)}\). Then
\[
\sum_{i=1}^{I_{t,j}} \mathbb{E}_q\!\big[(y_{t,j,i}-h_{t,j}^{(f)\top} x_t^{(f)})^2\big]
=
...
\[
\boxed{
\mathbb{E}_q[\mathrm{SSE}_j]\;{+}{=}\; S_{w,t,j} + I_{t,j}\big(\bar y_{t,j}-h_{t,j}^{(f)\top} m_t\big)^2 + I_{t,j}\,h_{t,j}^{(f)\top} C_t h_{t,j}^{(f)}.
}
\]
...
Fix a forecast time (t=T+k). Under Model C, let
[
\boxed{
x_t^{(f)} =
\begin{cases}
...
and define each product-specific latent corrected signal by
[
\boxed{
\mu_t^{(j)} := h_{t,j}^{(f)\top} x_t^{(f)},
\qquad j=1,\dots,J_f.
...
Thus the current no-transfer forecast model is recovered by taking ((x_t^{(f)},h_{t,j}^{(f)}))=((\beta_t,e_{t,j})), while the transfer-retaining forecast model uses ((x_t^{(f)},h_{t,j}^{(f)}))=((\widetilde\beta_t,\widetilde e_{t,j})).

#### (a) MCMC (exact Monte Carlo)

Given draws ({x_t^{(f),(s)}}_{s=1}^S),
[
\boxed{
\mu_t^{(j),(s)} := h_{t,j}^{(f)\top} x_t^{(f),(s)},
\qquad s=1,\dots,S.
}
]
...
If (q(x_t^{(f)})=N(m_t,C_t)), then the implied marginal for (\mu_t^{(j)}) is Normal:
[
\boxed{
\mu_t^{(j)} \ \big|\ \mathcal{D} \ \approx\ N!\left(
h_{t,j}^{(f)\top} m_t,\ h_{t,j}^{(f)\top} C_t h_{t,j}^{(f)}
\right)
\quad\text{under }q.
}
]
...
[
y_T^{j,i}(k)\mid x_t^{(f)},\sigma_j^2 \sim N\!\left(h_{t,j}^{(f)\top} x_t^{(f)},\ \sigma_j^2\right),
\qquad i=1,\dots,I_j.
]

#### (a) MCMC posterior predictive (exact mixture)

For a new replicate member value (y_t^{j,\mathrm{rep}}) at ((t,j)),
[
\boxed{
y_t^{j,\mathrm{rep}} \mid \mathcal{D}
\approx
...
[
\widehat{\mathbb{E}}[y_t^{j,\mathrm{rep}}\mid\mathcal{D}]
=
\frac1S\sum_{s=1}^S \mu_t^{(j),(s)},
\qquad
\widehat{\mathrm{Var}}(y_t^{j,\mathrm{rep}}\mid\mathcal{D})
=
\frac1S\sum_{s=1}^S\Big(\sigma_j^{2,(s)}+(\mu_t^{(j),(s)})^2\Big)
-
\widehat{\mathbb{E}}[y_t^{j,\mathrm{rep}}\mid\mathcal{D}]^2.
]

#### (b) VB moment-matched Normal predictive

Using (q(x_t^{(f)})=N(m_t,C_t)) and (q(\sigma_j^2)=\mathrm{IG}(a_j,b_j)), report
[
\boxed{
y_t^{j,\mathrm{rep}} \mid \mathcal{D}\ \approx
N!\left(
h_{t,j}^{(f)\top} m_t,
h_{t,j}^{(f)\top} C_t h_{t,j}^{(f)} + \tilde\sigma_j^2
\right),
}
\qquad
...
To produce a forecast for the **target** at forecast time (t), define the reduced target design vector
[
\boxed{
h_t^{0,\mathrm{red}} :=
\begin{pmatrix}
...
[
\boxed{
\mu_t^{0,\mathrm{red}} := h_t^{0,\mathrm{red}\top}\beta_t = F_t^\top\theta_t.
}
]

If the transfer-function block is retained in the forecast period and enters additively through (\zeta_t), define the augmented target design vector
[
\boxed{
h_t^{0,\mathrm{aug}} :=
\begin{pmatrix}
...
[
\boxed{
\mu_t^{0,\mathrm{aug}} := h_t^{0,\mathrm{aug}\top}\widetilde\beta_t
=
h_t^{0,\mathrm{red}\top}\beta_t + \zeta_t.
}
]

Thus the synthesized target forecast is

**Reduced forecast specification**
[
\boxed{
y_t^0 \mid \beta_t,\sigma_0^2 \sim N\!\left(h_t^{0,\mathrm{red}\top}\beta_t,\ \sigma_0^2\right).
}
]

**Augmented forecast specification**
[
\boxed{
y_t^0 \mid \widetilde\beta_t,\sigma_0^2,x_t^\star
\sim N\!\left(h_t^{0,\mathrm{aug}\top}\widetilde\beta_t,\ \sigma_0^2\right).
}
]

In either case, the “synthesis” occurs because the forecast data (\mathcal{Y}_{T+k}) update the posterior of the latent baseline and discrepancy components through the filtering/smoothing step in Model C. If (x_t^\star=\widehat x_{t\mid T}) is a deterministic forecast path, then the augmented synthesized forecast is conditional on that supplied path and does not propagate uncertainty in the covariate forecasts.
